...
class Packets(object):
    def __init__(self, protocol):
        ...
        # return because the code below is for special purpose use
        return
        #    """to use this, convert the class to a function, re-indent
        #    correctly, then replace all `self.` with `__`.  Run entire
        #    module. Then undo after you garner the output."""
        ...
        file = b"# {76: ('ENTITY_TELEPORT', '0x4c')}\n"
        reorder = copy.deepcopy(pkt_temp)
        for each in reorder:
            print(each)
            print(reorder[each])
            file += b"%d = %s (%s)\n" % (
                each,
                bytes(reorder[each][0], encoding="utf-8"),
                bytes(reorder[each][1], encoding="utf-8")
            )

        with open("/home/surest/Desktop/CB.pkl", "wb") as f:
            pickle.dump(pkt_temp, f, protocol=0)

        with open("/home/surest/Desktop/CB.txt", "wb") as f:
            f.write(file)


# Run the class as a function to create contants in `NN = "whatever"` order...
