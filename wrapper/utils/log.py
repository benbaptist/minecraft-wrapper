...
class Log:
    ...
    """
    def info(self, message, exec_info=False):
        logging.info(message, exec_info, extra={'plugin':self.plugin})

    """
        Detailed information, typically of interest only when diagnosing problems.
    """
    def debug(self, message, exec_info=False):
        if Config.debug:
            debug_style = termcolors.make_style(fg='cyan')
            logging.debug(debug_style(message), exec_info, extra={'plugin':self.plugin})

    def trace(self, message, exec_info=False):
        if Config.trace:
            trace_style  = termcolors.make_style(fg='magenta')
            logging.trace(trace_style(message), exec_info, extra={'plugin':self.plugin})
    ...
    """
    def warn(self, message, exec_info=False):
        warn_style = termcolors.make_style(fg='yellow', opts=('bold',))
        logging.warn(warn_style(message), exec_info, extra={'plugin':self.plugin})

    """
        Due to a more serious problem, the software has not been able to perform some function.
    """
    def error(self, message, exec_info=False):
        error_style = termcolors.make_style(fg='red', opts=('bold',))
        logging.error(error_style(message), exec_info, extra={'plugin':self.plugin})
    ...
    """
    def exception(self, message, exec_info=True):
        except_style = termcolors.make_style(fg='red', opts=('bold',))
        logging.exception(except_style(message), exc_info, extra={'plugin':self.plugin})

    """
        A serious error, indicating that the program itself may be unable to continue running.
    """
    def critical(self, message, exec_info=False):
        crit_style = termcolors.make_style(fg='black', bg='red', opts=('bold',))
        logging.critical(crit_style(message), exec_info, extra={'plugin':self.plugin})
