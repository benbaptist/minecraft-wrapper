import socket, datetime, time, sys, threading, random, subprocess, os, json, signal, traceback
class Server:
				...
				if self.backupInterval == self.config["Backups"]["backup-interval"] and self.config["Backups"]["enabled"]:
					...
					if self.config["Backups"]["backup-notification"]:
						if statusCode == 0:
							...
							pass
							# I disabled this error-message because it seemed like it always went off for no reason
