    ...
    def playerCommand(self, payload):
            ...
            if commandName in self.commands[pluginID]:
                try:
                    ...
                    payload["player"].message(
                        {"text": "An internal error occurred on the server side while trying to execute this command. Apologies.", "color": "red"})
                    return False
            ...
            if len(args(payload["args"], 0)) > 0:
                ...
                if subcommand == "update":
                    ...
                    if update:
                        ...
                        player.message(
                            "&cNo new Wrapper.py versions available.")
                    ...
                    if self.wrapper.server.getMemoryUsage():
                        player.message("&cServer Memory: %d bytes" %
                                       self.wrapper.server.getMemoryUsage())
                    else:
                        player.message(
                            "&cError: Couldn't retrieve memory usage for an unknown reason")
                elif subcommand == "random":
                    player.message("&cRandom number: &a%d" %
                                   random.randrange(0, 99999999))
            else:
                player.message({"text": "Wrapper.py Version %s" % (
                    buildString), "color": "gray", "italic": True})
            return
        if payload["command"] in ("plugins", "pl"):
            if player.isOp():
                ...
                for id in self.wrapper.plugins:
                    ...
                    if plugin["good"]:
                        player.message({"text": name, "color": "dark_green", "hoverEvent": {"action": "show_text", "value": "Filename: %s | ID: %s" % (
                            plugin["filename"], id)}, "extra": [{"text": " v%s" % version, "color": "dark_gray"}, {"text": " - ", "color": "white"}, summary]})
                    else:
                        player.message({"text": name, "color": "dark_red", "extra": [{"text": " - ", "color": "white"}, {
                                       "text": "Failed to import this plugin!", "color": "red", "italic": "true"}]})
                return False
        if payload["command"] == "reload":
            if player.isOp():
                ...
                try:
                    ...
                    player.message(
                        {"text": "Plugins reloaded.", "color": "green"})
                    if self.wrapper.server.getServerType() != "vanilla":
                        player.message(
                            {"text": "Note: If you meant to reload the server's plugins and not Wrapper.py's plugins, run `/reload server`.", "color": "gold"})
                except:
                    self.log.error("Failure to reload plugins:")
                    self.log.error(traceback.format_exc())
                    player.message(
                        {"text": "An error occurred while reloading plugins. Please check the console immediately for a traceback.", "color": "red"})
                return False
        ...
        if payload["command"] in ("ban-ip", "pardon-ip"):
            if player.isOp():
                if self.wrapper.config["Proxy"]["proxy-enabled"]:
                    player.message(
                        {"text": "Proxy mode!", "color": "dark_purple"})
                    player.message(
                        {"text": "Note: This will not work in proxy mode", "color": "red"})
                    player.message(
                        {"text": "In proxy mode, use /proxy-ban-ip or /proxy-pardon-ip.", "color": "green"})
                    return False
        if payload["command"] == "proxy-ban-ip":
            if player.isOp():
                if self.wrapper.config["Proxy"]["proxy-enabled"]:
                    ...
                    player.message(
                        {"text": "This will only work in proxy mode", "color": "red"})
                    player.message(
                        {"text": "The server is not in proxy mode (use /ban-ip).", "color": "red"})
                    return False
        ...
        if payload["command"] in ("help", "?"):
            ...
            def showPage(page, items, command, perPage):
                ...
                if pageCount > 1:
                    if page > 0:
                        prevButton = {"text": "Prev", "underlined": True, "clickEvent": {
                            "action": "run_command", "value": "%s %d" % (command, page)}}
                    else:
                        prevButton = {"text": "Prev",
                                      "italic": True, "color": "gray"}
                    if page <= pageCount:
                        nextButton = {"text": "Next", "underlined": True, "clickEvent": {
                            ...
                            if groupName.lower() == group:
                                ...
                                for i in group:
                                    ...
                                    if len(i) > 1 and player.isOp():
                                        permission = {"text": "Requires permission '%s'." % i[
                                            2], "color": "gray", "italic": True}
                                    items.append({"text": "", "extra": [
                                        {"text": command, "color": "gold", "clickEvent": {
                                            "action": "suggest_command", "value": command}, "hoverEvent": {"action": "show_text", "value": permission}},
                                        {"text": " " + args,
                                            "color": "red", "italic": True},
                                        {"text": " - %s " % i[1]}
                                    ]})
                                showPage(page, items, "/help %s" %
                                         groupName, 4)
                                return
                    player.message(
                        "&cThe help group '%s' does not exist." % group)
            else:
                items = []
                for v in helpGroups:
                    items.append({"text": "", "extra": [
                        {"text": v["name"], "color": "gold", "clickEvent": {
                            "action": "run_command", "value": "/help " + v["name"]}},
                        {"text": " - " + v["description"]}
                    ]})
            ...
            if player.isOp():
                ...
                for uu in players:
                    ...
                    for i in players[uu]["logins"]:
                        totalPlaytime[playerName][
                            0] += players[uu]["logins"][i] - int(i)
                ...
                if command == "groups":
                    ...
                    if subcommand == "new":
                        self.wrapper.permissions["groups"][
                            group] = {"permissions": {}}
                        player.message(
                            "&aCreated a new permissions group '%s'!" % group)
                        ...
                        player.message(
                            "&aDeleted permissions group '%s'." % group)
                    elif subcommand == "set":
                        if group not in self.wrapper.permissions["groups"]:
                            player.message(
                                "&cGroup '%s' does not exist!" % group)
                        ...
                        if len(node) > 0:
                            self.wrapper.permissions["groups"][
                                group]["permissions"][node] = value
                            player.message(
                                "&aAdded permission node '%s' to group '%s'!" % (node, group))
                        else:
                            usage(
                                "groups %s set <permissionNode> [value]" % group)
                        ...
                        if node in self.wrapper.permissions["groups"][group]["permissions"]:
                            ...
                            player.message(
                                "&aRemoved permission node '%s' from group '%s'." % (node, group))
                        ...
                        for uuid in self.wrapper.permissions["users"]:
                            if group in self.wrapper.permissions["users"][uuid]["groups"]:
                                player.message("%s: &2%s" % (
                                    self.wrapper.lookupUsernamebyUUID(str(uuid)), str(uuid)))
                        player.message(
                            "&aPermissions for the group '%s':" % group)
                        ...
                        player.message("&cList of groups: %s" % ", ".join(
                            self.wrapper.permissions["groups"]))
                        usage(
                    ...
                    if subcommand == "group":
                        ...
                        if len(group) > 0:
                            ...
                            if group not in self.wrapper.permissions["users"][uuid]["groups"]:
                                self.wrapper.permissions["users"][
                                    uuid]["groups"].append(group)
                                player.message(
                                    "&aAdded user '%s' to group '%s'!" % (username, group))
                            else:
                                self.wrapper.permissions["users"][
                                    uuid]["groups"].remove(group)
                                player.message(
                                    "&aRemoved user '%s' from group '%s'!" % (username, group))
                        else:
                        ...
                        if len(node) > 0:
                            self.wrapper.permissions["users"][
                                uuid]["permissions"][node] = value
                            player.message(
                                "&aAdded permission node '%s' to player '%s'!" % (node, username))
                        else:
                            usage(
                        ...
                        if node in self.wrapper.permissions["users"][uuid]["permissions"]:
                            del self.wrapper.permissions["users"][
                                uuid]["permissions"][node]
                            player.message(
                                "&aRemoved permission node '%s' from player '%s'." % (node, username))
                            return
                    elif subcommand == "info":
                        player.message(
                            "&aUser '%s' is in these groups: " % username)
