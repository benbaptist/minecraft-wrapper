import socket, threading, struct, StringIO, time, traceback, json, encryption, random, requests, hashlib
class Proxy:
...
#						print data
				...
				if objection:
					if self.sendCipher:
						print "Packing Encrypted from server to client"
						self.client.send(self.sendCipher.encrypt(packet[2]))
					else:
						print "Sending Unencrypt"
		...
		try:
			while not self.abort:
				...
				try:
					...
					self.disconnect()
					break
				if self.state is not 3:
					objection = False
				else:
					print "Packet not objected!"
...
#						data = self.read("string:address", packet[1])
						...
						self.send(0x00, "string", ('{"description":"Servers","players":{"max":20,"online":0},"version":{"name":"14w26c","protocol":25}}',), self.client, client=True)
						objection = False
					elif self.state == 2:
...
#				 		verifyToken = "".join([ch() for i in range(4)])
						...
						print "responded to server-side ping packet"
				if packet[0] == 1:
					if self.state == 1:
						...
						self.send(0x00, "string", (self.username,), self.server)
				 		self.send(0x02, "string|string", ("8e9a15c6-98c2-4cb8-8184-8d845085a846", "benbaptist"), self.client, client=True)
				 		#self.send(0x01, "int|ubyte|byte|ubyte|ubyte|string", (5, 0, -1, 3, 50, "default"), self.client, client=True)
				 		#self.send(0x08, "double|double|double|float|float|byte", (5, 200, 5, .4, .2, 0), self.client, client=True)
				 		self.send(0x02, "string|byte", (json.dumps({"text":"HELLO THERE LOL"}),0), self.client, client=True)
				 		objection = False
					elif self.authed:
						data = self.read("string:message", packet[1])
						self.send(0x02, "string|byte", (json.dumps({"text":"<%s> %s" % (self.username, data["message"])}),0), self.client, client=True)
						if data is not None:
							try:
