...
class Main:
    ...
    def onEnable(self):
        ...
        self.api.registerHelp(
            "Bookmarks", "Commands from the Bookmarks plugin", [
                ("/bmset <name>",
                 "Save current position as bookmark", "bookmarks"),
    ...
    def _isallowed(self, cmd, player, args):
        ...
        if not player.getDimension() == 0:
            player.message(
                "&cSorry, but you can't do this from the Nether or End."
            )
            return False
        if not player.isOp() and not player.getGamemode() == 0:
            player.message(
                "&cSorry, but you can do this only in Surival gamemode."
            )
            return False
        bmname = args[0]
        if player.username not in self.data.Data:
            if cmd in ["bmgo", "bmdel"]:
                player.message(
                    {"text": "You haven't used bookmarks yet.\n",
                     "color": "red", "extra": [
                        {"text": " Use ", "color": "gray"},
                        {"text": "/bmset", "color": "gold",
            ...
            if bmname in self.data.Data[player.username]:
                if cmd in ["bmgo", "bmdel"]:
                    ...
                    player.message(
                        {"text": "Bookmark ", "color": "red", "extra": [
                            {"text": bmname, "color": "dark_red"},
                            {"text": " is already saved.\n", "color": "red"},
                            {"text": " Use ", "color": "gray"},
                            {"text": "/bmdel %s" % bmname, "color": "gold",
                ...
                if cmd in ["bmgo", "bmdel"]:
                    ...
                    if len(self.data.Data[player.username]) >= 5:
                        player.message(
                            {"text": "Maximum number of bookmarks already in use.\n",  # noqa
                             "color": "red", "extra": [
                                {"text": " Remove some of them with ",
                                 "color": "gray"},
                                {"text": "/bmdel", "color": "gold",
                                 "clickEvent": {"action": "suggest_command",
                                                "value": "/bmdel"}},
                                {"text": " <name>", "color": "gold",
                                 "italic": True},
                                {"text": " first.\n Use ", "color": "gray"},
                                {"text": "/bmlist", "color": "gold",
                                 "clickEvent": {"action": "run_command",
                                                "value": "/bmlist"}},
                                {"text": " to see their names.",
                                 "color": "gray"},
                              ]
                             }
                        )
    ...
    def bookmarkset(self, player, args):
        ...
        player.message(
            {"text": "Bookmark ", "color": "green", "extra": [
                {"text": bmname, "color": "dark_green"},
                {"text": " set.\n", "color": "green"},
                {"text": " Use ", "color": "gray"},
                {"text": "/bmgo %s" % bmname, "color": "gold",
                    "clickEvent": {"action": "run_command",
                                   "value": "/bmgo %s" % bmname}},
    ...
    def bookmarkgo(self, player, args):
        ...
        bmname = args[0]
        player.message(
            {"text": "Teleporting you to your bookmark ", "color": "green",
             "extra": [
                 {"text": bmname, "color": "dark_green"},
                 {"text": ".", "color": "green"},
             ]
        ...
        self.api.minecraft.console(
            "tp %s %s %s %s" % (
                player.username, self.data.Data[player.username][bmname][0],
    ...
    def bookmarklist(self, player, args):
        ...
        if len(args) != 0:
            player.message(
                {"text": "Use ", "color": "gray", "extra": [
                    {"text": "/bmlist", "color": "gold",
                     "clickEvent": {
                         "action": "run_command", "value": "/bmlist"
                     }
                     },
                  ]
                 }
            ...
            player.message(
                {"text": "You haven't used bookmarks yet.\n", "color": "red",
                 "extra": [
                     ...
                     {"text": "/bmset", "color": "gold",
                      "clickEvent": {
                          "action": "suggest_command", "value": "/bmset"
                      }
                      },
                     {"text": " <name>", "color": "gold", "italic": True}
                 ]
        ...
        for bmname in self.data.Data[player.username].keys():
            bmlist.append(
                {"text": " %s" % bmname, "color": "gold",
                 "clickEvent": {"action": "run_command",
                                "value": "/bmgo %s" % bmname}
                 }
            )
        player.message(
            {"text": "Your bookmarks are:", "color": "green", "extra": bmlist}
        )
