...
class Window():
	def __init__(self,player,title,slots):
		pass
...
class Main:
	...
	def onEnable(self):
		...
		("/addc <WNAME>, <COMMAND> [ARG1 [ARG2]]","Adds a new command to the menu named WNAME for the held item with the command COMMAND and args ARG1, ARG2, etc.","s.wop"),
	...
	def removeCommand(self,player,args):
		if len(args)<2: player.message("USAGE: /remc <WNAME>");return
		w=self.storage[args[0]]
		item=player.getHeldItem()
		for i in range(len(w)):
			if w[i["item"]]==item: w[i]=None
