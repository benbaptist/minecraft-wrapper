...
class Main:
    ...
    def IRCCommand(self, payload):
        def args(i):
            try:
                return payload["message"].split(" ")[i]
            except:
                return ""

        # def argsAfter(i):
        #     try:
        #         return " ".join(payload["message"].split(" ")[i:])
        #     except:
        #         return ""
        try:
            if not payload["message"][0] == "!":
                return
        except:
            return
        command = args(0)[1:]
    ...
    def command(self, payload):  # sloppy debug stuff. :P
                ...
                if poll in self.data["polls"]:
                    if len(value) > 0:
                        try:
                            value = int(value)
                            pollObject = self.data["polls"][poll]
                            if -1 < value < len(pollObject["options"]):
                                if player.username in pollObject["results"]:
                                    player.message("&a&lChanged your vote from '&o&6%s&r&a&l' to '&o&6%s&r&a&l'!" %
                                                   (pollObject["options"][pollObject["results"][player.username]],
                                                    pollObject["options"][value]))
                                else:
                                    player.message("&a&lThanks for voting on '&o&6%s&r&a&l'!" %
                                                   pollObject["options"][value])
                        ...
                        player.message("&aTo vote on one of the options, do !vote %s 0-%d" %
                                       (poll, len(self.data["polls"][poll]["options"]) - 1))
                else:
                    player.message("&cError: Poll '%s' does not exist!" % poll)
            else:
        ...
        if command == "setpoll" and player.isOp():
            ...
            if len(poll) > 0 and len(options) > 0:
                ...
                player.message("&cThe options are separated by commas. &l&opoleName is case sensitive "
                               "and cannot have spaces!&r&c Best to make it undercase.")

    def join(self, payload):
        name = payload["player"]
        player = self.minecraft.getPlayer(name)
        if self.data["broadcast_new_polls"]:
            if not self.data["latest_poll"] is None:
                player.message("&a&lWelcome back, &6%s&a.&r&a Check out the latest poll: &3%s." %
                               (name, self.data["latest_poll"]))
